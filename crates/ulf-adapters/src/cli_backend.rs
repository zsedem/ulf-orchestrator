...
use ulf_core::{CliConfig, HatBackend};
use std::fmt;
use std::io::Write;
...
impl CliBackend {
    ...
    pub fn from_config(config: &CliConfig) -> Result<Self, CustomBackendError> {
        ...
        // This keeps ulf.yml `cli.args` consistent with CLI `-- ...` extra args behavior.
    ...
    ///
    /// Note: This is NOT for TUI mode - Ulf's TUI uses the standard `claude()`
    ...
    /// positional argument. Used by `ulf plan` for interactive sessions.
    ...
    /// as a positional argument. Used by `ulf plan` for interactive sessions.
...
mod tests {
    ...
    fn test_opencode_interactive_launches_tui_not_headless() {
        // Issue #96: opencode backend doesn't start interactive session with ulf plan
        //
    ...
    fn test_custom_args_can_be_appended() {
        ...
        // This is used for `ulf run -b opencode -- --model="some-model"`
