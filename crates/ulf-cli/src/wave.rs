...
//!
//! Provides `ulf wave emit` for agents to dispatch work items
...
///
/// Priority: ULF_EVENTS_FILE env > .ulf/current-events marker > default .ulf/events.jsonl
pub fn resolve_events_file() -> PathBuf {
    if let Ok(path) = std::env::var("ULF_EVENTS_FILE") {
        return PathBuf::from(path);
    }
    fs::read_to_string(".ulf/current-events")
        .map(|s| PathBuf::from(s.trim()))
        .unwrap_or_else(|_| PathBuf::from(".ulf/events.jsonl"))
}
