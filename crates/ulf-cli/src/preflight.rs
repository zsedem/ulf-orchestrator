...
use ulf_core::{CheckResult, CheckStatus, PreflightReport, PreflightRunner, UlfConfig};
...
pub(crate) async fn load_config_for_preflight(
    ...
    let mut config: UlfConfig = serde_yaml::from_value(core_value)
...
fn validate_core_config_shape(value: &Value, label: &str) -> Result<()> {
    ...
    if mapping_get(mapping, "project").is_some() {
        anyhow::bail!(ulf_core::ConfigError::DeprecatedProjectKey);
    }
...
mod tests {
    ...
    fn config_source_label_handles_sources() {
        ...
        let expected_core =
            crate::config_resolution::compose_core_label(user_label.as_deref(), "ulf.yml", false);
        assert_eq!(
            with_hats_label,
...
",
        ...
        let config: UlfConfig = serde_yaml::from_value(merged).unwrap();
...
",
    ...
    async fn load_config_for_preflight_hats_source_takes_precedence_over_core_hats() {
        ...
        let core_path = temp_dir.path().join("ulf.yml");
