# Rough Idea: Add roo-cli as a Provider to Ulf Orchestrator

## What
Add `roo` (Roo Code CLI) as a new backend provider in ulf-orchestrator, enabling `ulf run -b roo` and `ulf plan -b roo`.
...
5. **Interactive mode**: `roo "prompt"` for `ulf plan`.
6. **Auto-detect last** in priority: `["claude", "kiro", ..., "pi", "roo"]`.
7. **Roo always exits code 0** — Even on API errors. Ulf relies on LOOP_COMPLETE/event presence.
8. **Event tags work** — Roo emits `<event topic="build.done">` and `LOOP_COMPLETE` when Ulf-style system instructions are provided.
...
## Files to Modify
1. `crates/ulf-adapters/src/cli_backend.rs` — Add `roo()`, `roo_interactive()`, registration in `from_config`, `from_name`, `for_interactive_prompt`
2. `crates/ulf-adapters/src/auto_detect.rs` — Add "roo" to `DEFAULT_PRIORITY`, update `NoBackendError`
3. `crates/ulf-adapters/src/lib.rs` — Update doc comment
